...
// ES6 class
class BarChart {
  constructor(id, data, outerWidth, outerHeight, titleText) {
    this.id = id;
    this.data = data;
    this.keys = this.data.map((d) => {
      return d.key;
    });
    this.values = this.data.map((d) => {
      return d.value;
    });
...
export default function(id = 'chart1', data = d3.range(10),
  width = OUTER_WIDTH, height = OUTER_HEIGHT, title) {
  return new BarChart(id, data, width, height, title);
}
